...
* [Fix] `xschem` no longer asks whether embedded Tcl scripts may be executed, which the PDK launcher symbols and `tcleval()` attributes need in order to work silently. `xschem_execute_scripts yes` is applied centrally in the system-wide `xschemrc` and is therefore active for all PDKs and in every directory. Setting it in the shipped user `xschemrc` was not enough, as that file is read far less often than it appears: `xschem` always sources the system-wide file first, and then exactly one of `./xschemrc` in the directory it was started in or `~/.xschem/xschemrc`, whichever it finds first. A design directory carrying its own `xschemrc`, which every IIC design template does, therefore shadows the user file completely, and the setting was skipped in exactly the sessions that need it.
...
* [Fix] the IHP `SG13CMOS5L` capacitor corners work in VACASK again. Both IHP PDKs are installed from a branch rather than from a pinned commit, and on 2026-08-11 they gained `cap_cmomf`, the metal fringe MoM capacitor. `cornerCAP.lib` gained an `.include cap_cmomf.lib` in every one of its sections, but VACASK's `sg13cmos5ltovc.py` converter names the model files it converts and the Verilog-A it compiles in two hardcoded lists, so the new device was skipped while the corner file including it was converted verbatim — and every VACASK deck pulling in that corner died on the missing include, even one using none of its devices (regression test 29). The converter's device lists are now completed from the installed PDK before it runs, deriving the devices from the PDK tree instead of naming them, so the next device it gains is covered as well; the fix is a no-op once VACASK lists them itself. Two install-time checks back it up: every include in the converted VACASK model files has to resolve, and every OSDI object the PDK's own `.spiceinit` loads has to exist — the latter also catches a Verilog-A model that failed to compile, which the previous check missed because it only looked for `cap_cmomi.osdi`.
* [Fix] the pcell inventory baselines of regression test 27 are updated for the same PDK bump: `ihp-sg13g2` goes from 37 to 38 pcells (`cap_cmomf`) and `ihp-sg13cmos5l` from 24 to 26 (`cap_cmomf` and `guard_ring`). The counts are read off the pcell libraries the PDKs register; no pcell is pinned as expected-empty, so the test reports the new devices by name should any of them fail to produce geometry.
