...
* [Adding] [`gdscheck`](https://github.com/aesc-silicon/gdscheck) `v0.1.2`, a DRC engine for GDSII layouts written in Rust. It reads GDS directly, flattens the cell tree itself and needs neither KLayout nor Magic nor a foundry toolchain; rules are plain YAML decks, the checks fan out across all cores, and violations are written as a KLayout-compatible `.lyrdb` marker database that loads straight into the marker browser. The decks for IHP `SG13G2` and `SG13CMOS5L` ship inside the binary, so `gdscheck run --process ihp-sg13g2 ...` works without any external files. Note that upstream calls it experimental with incomplete coverage, so cross-check against Magic or KLayout before signoff.
* [Adding] FPGA place-and-route for Lattice ECP5 next to the existing Lattice iCE40 flow: `nextpnr` is now built for both architectures in one pass, and [`prjtrellis`](https://github.com/YosysHQ/prjtrellis) supplies `ecppack` and the ECP5 database. This completes the in-container build chain for the ULX3S board of the AMS template's FPGA flow, at a cost of roughly 95 MB (the ECP5 part of the Trellis database dominates, and the unused MachXO families are dropped at install time). Programming the board stays on the host, since the container has no USB access. New test 33 takes an iCE40 and an ECP5 blinky through synthesis, place-and-route and bitstream packing.
...
* [Update] the Yosys stack (`yosys`, `eqy`, `sby`, `mcy`) is at `v0.68`, together with `librelane` `3.1.0.dev3`. `v0.68` removed the `abc` pass's `-fast` option, which older LibreLane called unconditionally, so both have to move together.
* [Update] various tool and Python package version bumps: `bender`, `gdsfactory`, `ghdl`, `gtkwave`, `iverilog`, Kactus2 (`v3.14.1`), `kepler-formal`, KLayout (`v0.30.10`), `klayout-pex`, Magic (`8.3.681`), `najaeda`, `nextpnr`, `ngspice` (`ngspice-47`), `nvc` (`r1.22.1`), `open_pdks`, `openEMS`, `openvaf`, OpenROAD, `palace`, the RISC-V GNU toolchain, `scikit-rf`, `setupEM`/`gds2palace`, SiliconCompiler, `slang` and the `slang-yosys-plugin`, `snp2le`, `spicebind`, `spike`, `surelog` (`v1.87`), `sv2v`, `uv` (`0.12.5`), VACASK, `verible`, and `xschem`.
...
* [Fix] the `xschem` menus and launchers of `gf180mcuD`, `ihp-sg13g2` and `ihp-sg13cmos5l` called shell commands (`mkdir -p`, `python3`) from Tcl, so "Create FET .save file" and the SimulatePARALLEL launchers died with `invalid command name` whenever `xschem` was started detached (`sak-open`, desktop entry, `xschem &`). Patched at PDK-install time.
* [Fix] VACASK aborted on host kernels older than 5.3 (RHEL/Rocky/Alma 8) for every deck with a `postprocess(PYTHON, ...)`, since Boost.Process v2 uses `pidfd_open` with no fallback. Boost and VACASK are now built without it.
...
* [Docs] `KNOWN_ISSUES.md` records the KLayout `0.30.10` deep-mode DRC regression: separation and enclosure violations that neither `0.30.9` nor flat mode reports, which is why test 04 fails. Workaround: `-rd run_mode=flat` ([KLayout issue 2423](https://github.com/KLayout/klayout/issues/2423)).
