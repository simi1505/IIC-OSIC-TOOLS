...
if command -v librelane >/dev/null 2>&1; then
    ...
    if [ -n "$FAILED" ]; then
        echo "[ERROR] Test <LibreLane smoke-test using VHDL with sky130A> FAILED ($FAILED). Check the logs <$LOG> and <$STDERR_LOG>."
        exit 1
    else
    ...
    echo "[ERROR] Test <LibreLane smoke-test using VHDL with sky130A> FAILED. LibreLane is not installed!"
    exit 1
fi
