...
if [ "$(${CONTAINER_ENGINE} ps -aq -f name="${CONTAINER_NAME}")" ]; then
	...
	if [[ $k = s ]] ; then
		...
		if ! check_container_alive 8; then
			selinux_failure_hint
			exit 1
		fi
		check_container_log_errors
	...
	if ! ${ECHO_IF_DRY_RUN} "${CONTAINER_ENGINE}" run -d --user "${CONTAINER_USER}:${CONTAINER_GROUP}" -e "DISPLAY=${DISP}" -v "${DESIGNS}":"/foss/designs":rw ${PARAMS} --name "${CONTAINER_NAME}" "${IMAGE_NAME}"; then
		echo "[ERROR] Could not create the container ${CONTAINER_NAME}."
