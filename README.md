...
- `DOCKER_USER="hpretl"` username for the Docker Hub repository from which the images are pulled. Usually, no change is required. To pull from a private registry, set it to the registry (e.g. `DOCKER_USER="myregistry.example.com:5000"`, optionally followed by a namespace such as `ghcr.io/hpretl`).
